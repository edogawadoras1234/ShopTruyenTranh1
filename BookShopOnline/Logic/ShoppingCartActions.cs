...
namespace BookShopOnline.Logic
{
    public class ShoppingCartActions : IDisposable
    {
       ...
       CartItemUpdates)
        {
            using (var db = new BookShopOnline.Models.BookContext())
            {
                try
                {
                    ...
                    foreach (var cartItem in myCart)
                    {
                        ...
                        {
                            if (cartItem.Book.MaSach == CartItemUpdates[i].BookId)
                            {
        ...
        {
            using (var _db = new BookShopOnline.Models.BookContext())
            {
                try
                {
                    var myItem = (from c in _db.ShoppingCartItems
                                  where c.CartId == removeCartID && c.Book.MaSach ==
                                 removeBookID
        ...
        quantity)
        {
            using (var _db = new BookShopOnline.Models.BookContext())
            {
                ...
                {
                    var myItem = (from c in _db.ShoppingCartItems
                                  where c.CartId == updateCartID && c.Book.MaSach ==
                                 updateBookID
                                  select c).FirstOrDefault();
                    if (myItem != null)
                    {
